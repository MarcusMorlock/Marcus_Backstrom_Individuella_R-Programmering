...
# Read csv and turn it into a data frame
df_raw <- load_data("../data/insurance_costs.csv")

#look at the data but skip customer_id in unique 
...
# Clean the data frame and create age_group variable which puts ages into 4 groups.
...
#create bar chart for age group by charges
...
#Create summery of risk_level by charges with avg, median, min , max.
...
## 3.4 charges prior
```{r}
# plan type summery with charges, prior claims , annual checkup and prior accidents.
...
# List of the r2 value of each variable against charges individually.
...
```{r}
# regression model based on target = charges and choices. excluding bmi_category, age_group, health_risk_level.
